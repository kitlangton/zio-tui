...
class EscapeCodes(out: OutputStream) {
  ...
  // Output a Control Sequence Introducer
  ...
  /**
   * Executes a request and parses the response report. Usually, they would
   * start with a CSI but JLine seems to ignore them.
   * @param csi
   *   CSI to execute
   * @param args
